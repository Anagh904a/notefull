...
async function startUpdate() {
  ...
  showToast("This may take a few minutes");
  ...
  try {
    ...
    const minDownloadDisplay = 8000;
    const remainingTime = Math.max(0, minDownloadDisplay - downloadTime);
...
function startInitialProgress() {
  updateProgressInterval = setInterval(() => {
    if (!isDownloading && updateProgress < 15) {
      updateProgress += 0.3;
      updateProgressUI();
    } else if (isDownloading && !downloadComplete && updateProgress < 60) {
      updateProgress += 0.15;
      updateProgressUI();
    } else if (downloadComplete && updateProgress < 90) {
      updateProgress += 0.4;
      updateProgressUI();
    }
  }, 100);
